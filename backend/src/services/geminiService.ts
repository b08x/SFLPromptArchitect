...
class GeminiService {
  ...
  async testPrompt(promptText: string): Promise<string> {
    try {
      // Get API key for Google provider
      const apiKey = await secretsManager.getProviderApiKey('google');
      ...
      throw new Error("An unknown error occurred while contacting the AI API.");
    }
  }
  ...
  async generateSFLFromGoal(goal: string, sourceDocContent?: string): Promise<Omit<PromptSFL, 'id' | 'createdAt' | 'updatedAt'>> {
    // API key will be fetched from secrets manager when making the request
